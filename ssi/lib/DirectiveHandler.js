...
module.exports = (function () {
	...
	DirectiveHandler.prototype = {
		...
		handleDirective: function (directive, directiveName, currentFile, variables) {
			...
			errorContext = { currentFile, variables, directive, directiveName, attributes }
			...
			switch (directiveName) {
				...
				case "config":
					interpolate.apply(this);
					return this._handleConfig(attributes, variables);
				case "include":
					interpolate.apply(this);
		...
		_handleConfig: function (attributes, variables) {
			if (attributes.length == 1 && attributes[0].name === "timefmt") {
				if (attributes[0].value == '%Y%m%d%H%M%S') {
					function getApacheSSITime() {
						const now = new Date();
					...
					return { output: getApacheSSITime() };
				} else if (attributes[0].value == '%Y') {
					return { output: new Date().getFullYear() };
				}
				return { error: "Directive #config timefmt not handled" };
			}

			return { error: "Directive #config parsing error" };
		},
		...
		_handleElse: function () {
			if (!this._inConditional()) {
				console.warn(errorContext, { error: "Else while not inside of If block" });
